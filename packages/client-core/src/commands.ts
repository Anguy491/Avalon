...
export type RoomCommandInput =
  ...
  | { readonly type: 'SetReady'; readonly payload: { readonly ready: boolean } }
  ...
  | { readonly type: 'ContinuePhase'; readonly payload: Record<string, never> }
  ...
  | {
      ...
      readonly payload: { readonly choice: 'TERMINATE' | 'CONTINUE_PAUSE' };
    }
  | {
...
export class RoomCommandAttempts {
  ...
  acquire(
    ...
    if (this.pending?.fingerprint === fingerprint) return this.pending.command;
    const command = {
      commandId: createId(),
      roomId: roomView.public.roomId,
      expectedStateVersion: roomView.public.stateVersion,
  ...
  complete(commandId: string): void {
    if (this.pending?.command.commandId === commandId) this.pending = undefined;
  }
}
