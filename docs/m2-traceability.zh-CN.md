...
| `M2-003` | `SM-003`、`FR-006`、`NFR-010`、`TM-002`、`AC-019` | 256 位 token、仅摘要入库、事务轮换/撤销；AES-GCM 幂等响应；SecureStore；微信 `wx.login + SessionToken`、5 分钟内存身份令牌与同房主体唯一约束 | 服务端轮换/并发/旧 token、code2Session、错身份和同房冲突测试；移动端 SecureStore、微信身份刷新/存储检查、写入前崩溃和幂等键单测；secret scan |
