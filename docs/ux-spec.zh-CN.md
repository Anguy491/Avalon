...
iOS/Android 原生界面支持简体中文和英文，不提供应用内语言开关。启动及每次回到前台时按系统语言偏好顺序选择首个 `zh-*` 或 `en-*`，所有中文变体共用简体中文文案，无匹配语言时回退英文。iOS 同时遵循系统的单 App 语言设置。语言变化只重新渲染文案，不清理或重建会话、不改变命令和投影。

微信小程序保持仅中文。应用名称“曼波阿瓦隆”不翻译；相机系统权限说明分别提供中文和英文。角色、阵营、阶段、胜因、错误码及成功提示必须由稳定语义值在渲染时翻译，禁止把中文展示文案作为状态或分支条件。
...
配置以卡片呈现“基础配置”“推荐配置”“自定义”。建房默认 5 人基础配置；切换自定义后逐个调整角色数量，草稿在模式切换时保留，角色数等于目标人数后才允许提交；服务器继续执行最终合法性校验。
...
- 身份主动揭示后可打开本人角色攻略；常规任务与刺杀阶段也提供同一私密入口。攻略使用可滚动私密弹窗，进入后台、暂停、终局或重同步时关闭，不在缺少 `selfRole` 时猜测内容。
...
首屏展示胜方和四种规范胜因之一；下方按座次翻开全部角色，标出刺杀目标（适用时），再展示任务与组队公开历史。正常胜负进入本页；暂停投票或暂停硬上限产生的 `ABORTED` 不揭示角色、不进入结果页，在终局回执后直接清理会话并返回首页。
...
- 连续暂停未满 60 秒：显示终止投票开放倒计时；到时任一在线玩家可经危险操作确认发起；
- 终止投票进行中：公开显示选民总数、提交总数与 30 秒倒计时，只向本人显示“可投/已提交/不在本轮名单”，不回显个人选择；
- 严格超过半数选民选择继续暂停时遮罩保留并重新显示 60 秒冷却；否则显示中性中止状态并返回首页；
...
英文界面继续使用 `zh-CN-v1` 中文主持音频；字幕按同一 `audioCueKey` 映射为英文。中文字幕仍以获批音频清单为准，语言切换不得触发自动重播。
...
| 身份揭示与攻略 | `FR-017`–`FR-020`、`FR-050`、`NFR-014` | `AC-008`、`AC-015`、`AC-018` | 每类知识视角、八角色攻略、后台遮罩、读屏 |
...
| 暂停恢复 | `FR-040`–`FR-049`、`NFR-005`–`NFR-007` | `AC-009`–`AC-011`、`AC-016` | 10 秒暂停、多原因、在线选民终止投票、60 分钟硬上限、旧版本 |
| 无障碍 | `NFR-018`–`NFR-020` | `AC-014` | 最大字体、读屏、无声、非颜色表达 |
| 原生本地化 | `FR-051` | `AC-020` | 中英文正常/错误/最大字体状态、系统与单 App 语言切换、中文音频配英文字幕、小程序中文回归 |
