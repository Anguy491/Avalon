...
| `FR-002` | 创建页必须让房主选择 5–10 人、基础/推荐配置或自定义配置；默认选择基础配置。 | 创建成功后房主占据第一个座次，看到 6 位房间号与二维码。 | `RULE-001`、`RULE-004`、`SM-001` |
...
| `FR-006` | 创建或加入成功后，应用必须按平台会话策略保存令牌。 | 原生端使用平台安全存储；微信小程序沙箱只保存 SessionToken 和无秘密恢复摘要，不保存 `RoomView` 或微信身份令牌；恢复时轮换 SessionToken。微信身份令牌只在内存保留 5 分钟；任何平台均不得把凭证写入剪贴板、URL 或日志。 | `SM-003`、`NFR-010`、`AC-019` |
...
| `FR-011` | 房主可切换基础配置、推荐配置或逐个选择自定义角色。 | 推荐配置严格使用规则文档第 8.2 节；自定义角色数必须等于目标人数，服务端执行最终合法性校验。 | 规则文档第 8 节、`SM-004` |
...
| `FR-050` | 身份揭示、常规任务与刺杀阶段应提供本人当前角色的静态攻略。 | 内容只能由 `PrivatePlayerProjection.selfRole` 选择；身份页仅在主动揭示后开放，暂停、终局、后台或缺少本人角色时不得显示。 | `RULE-007`、`NFR-014` |
...
| `FR-036` | MVP 只使用随应用发布的固定中文音频，不使用运行时 TTS。 | 动态昵称、队伍成员和票数仅显示在屏幕；音频包缺失时自动降级为字幕；英文界面仍播放中文音频并显示对应英文字幕。 | `RULE-021`、`NFR-015`、`FR-051` |
...
| ID | 需求 | 验收标准 | 追踪 |
| --- | --- | --- | --- |
| `FR-051` | iOS/Android Expo 应用必须支持简体中文与英文，并按系统语言偏好顺序选择首个中文或英文；所有 `zh-*` 使用简体中文，均不匹配时回退英文。微信小程序保持仅中文。 | 不提供应用内语言开关或语言持久化；应用回到前台即重新解析系统语言并保持当前房间会话。iOS 支持系统单 App 语言；网络 `locale: "zh-CN"`、`zh-CN-v1` 语音包、服务端、协议和小程序行为均不改变。所有可见文案、错误、角色/阶段/胜因、字幕及无障碍标签随语言更新。 | `AC-020`、`ADR-013` |
...
| ID | 要求 | 验证方式 |
| --- | --- | --- |
| `NFR-009` | 原生发布时支持 iOS 和 Android 当前主要版本及前两个主要版本，覆盖手机竖屏；微信小程序支持微信当前稳定版。 | 原生发布矩阵真机/云真机回归；小程序开发者工具、低端 Android 与 iPhone 真机回归。 |
| `NFR-010` | 原生 `SessionToken` 必须存储于 iOS Keychain 或 Android Keystore 支持的安全存储。微信小程序采用 `wx.login + SessionToken` 双因子绑定：SessionToken 使用沙箱存储并维持 30 分钟滑动 TTL，微信身份令牌仅内存持有、固定 5 分钟有效；创建、加入、恢复、读取投影和实时握手均须匹配两者，不提供无 SessionToken 找回。 | 静态/本地存储检查、身份过期刷新、错账号、同账号同房冲突、token 轮换/旧 token 撤销、抓包与真机后台预览测试。 |
...
| `NFR-017` | 不得采集通讯录、精确位置、麦克风内容、广告标识符或与核心组局无关的设备数据。微信身份标识仅用于保护当前会话，原始 OpenID、UnionID 与 `session_key` 不保存；不调用用户资料接口，不收集微信昵称或头像。 | SDK 清单、微信隐私保护指引、商店隐私声明、服务端日志/数据库检查和运行时权限检查。 |
...
| `NFR-022` | 客户端不得持久化其他玩家或本人的私密投影；原生端确需短暂角色缓存时必须加密并在房间过期或关闭时删除，微信小程序只允许内存持有当前投影。 | 本地文件检查和生命周期测试。 |
...
| `AC-017` | 分别以 5–10 人创建推荐配置房间 | 角色牌组逐项符合规则第 8.2 节，9 人为 6 善 3 恶；任务人数和保护轮仍符合第 5.2 节与 `RULE-014`。 |
| `AC-018` | 覆盖八个本人角色、缺失角色、后台、暂停和终局状态打开攻略 | 仅显示本人角色攻略；八角色内容完整，后台/暂停/终局立即关闭且公共投影不增加角色字段。 |
| `AC-019` | 两个微信账号分别复制或轮换同一座位的 SessionToken，并覆盖身份过期、同账号同房第二座位与跨房加入 | 只有原绑定微信身份与有效 SessionToken 同时存在时可恢复、读取和连接；错账号与同房第二座位被拒，身份过期以原幂等键刷新重试一次，跨房允许，原生客户端不受影响。 |
| `AC-020` | 在 `zh_CN` 与 `en_US` 的 iOS/Android 模拟器覆盖首页、建房/加入、字段错误、相机拒绝、大厅、身份/攻略、对局、暂停和结果，并在活跃会话中切换系统/单 App 语言后回前台 | 原生 UI、当前提示和全部无障碍文案立即使用所选语言，房间、命令和私密投影不变；英文界面播放同一中文音频并显示英文字幕。无匹配系统语言回退英文；微信小程序构建与中文回归不变。 |
...
| `RULE-007`、静态非裁决内容 | `PrivatePlayerProjection.selfRole` | `FR-050` | `AC-018` |
| 幂等与错误模型 | `Command`、`ErrorCode` | `FR-046`、`FR-047` | `AC-009` |
| 微信会话身份 | `SM-001`–`SM-003`、SessionContext | `FR-006`、`NFR-010`、`NFR-017` | `AC-019` |
| 原生界面本地化（不改变裁决） | 无状态机或协议变更 | `FR-051` | `AC-020` |
| 安全与隐私 | 公开/私密投影 | `NFR-010`、`NFR-012`–`NFR-017`、`NFR-022`–`NFR-023` | `AC-013`、`AC-015`、`AC-019` |

## 9. 发布门槛

MVP 进入生产环境前必须满足：

1. `AC-001`–`AC-020` 全部通过；
2. 三份文档的规则表、状态名、命令名和需求引用无悬空或冲突；
3. 负载测试达到 `NFR-004`，且秘密数据检查无发现；
4. 支持系统矩阵上的核心流程、断线恢复、字幕和屏幕阅读器回归通过；
5. 固定中文语音、中英文字幕完成内容审核、版本校验和损坏降级测试；
