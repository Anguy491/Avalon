...
> 文档状态：可上线 MVP 需求基线
>
> 目标平台：iOS、Android；微信小程序为并行开发与内部测试目标
>
...
| `FR-006` | 创建或加入成功后，应用必须按平台会话策略保存令牌。 | 原生端使用平台安全存储；微信小程序开发版只使用应用沙箱存储且不保存 `RoomView`，恢复时轮换；任何平台均不得把令牌写入剪贴板、URL 或日志。 | `SM-003`、`NFR-010` |
...
| ID | 要求 | 验证方式 |
| --- | --- | --- |
| `NFR-009` | 原生发布时支持 iOS 和 Android 当前主要版本及前两个主要版本，覆盖手机竖屏；微信小程序开发目标支持微信当前稳定版，公开发布矩阵在真实 AppID 就绪后确认。 | 原生发布矩阵真机/云真机回归；小程序开发者工具与 iPhone/Android 真机回归。 |
| `NFR-010` | 原生 `SessionToken` 必须存储于 iOS Keychain 或 Android Keystore 支持的安全存储；微信小程序开发/内部测试版可使用应用沙箱存储，但不得保存投影，恢复成功后必须轮换。小程序公开发布前必须完成本地存储专项安全复核并决定是否引入 `wx.login` 绑定和更短令牌寿命。 | 静态检查、越狱/Root 与微信沙箱风险说明、存储抽检和令牌轮换/旧 token 撤销集成测试。 |
