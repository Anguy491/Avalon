...
本目录保存协议 v1 的 bootstrap JSON Schema，使用 JSON Schema Draft 2020-12。它们与 [API/实时协议契约](../api-contract.zh-CN.md)共同定义线上结构，领域语义仍以[服务端状态机](../server-state-machine.zh-CN.md)为准。
...
- 创建 `packages/protocol` 前：本目录是结构基线，修改时必须同步协议文档；
- M0 后：`packages/protocol` 的 TypeScript Schema 是编辑源，本目录由 `pnpm protocol:generate` 导出；
