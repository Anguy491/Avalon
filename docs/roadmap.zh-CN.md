...
| `M1-006` | 暂停覆盖、音频副作用描述和不变量 | `RULE-020`–`RULE-022` |
...
| ID | 交付物 | 主要追踪 |
| --- | --- | --- |
| `M6-001` | 心跳、10 秒暂停、多暂停原因与 30 分钟过期 | `FR-040`–`FR-045` |
...
## 11. M7：安全、无障碍、容量与 Preview

### 目标

把功能完整构建提升到满足发布门槛的 Preview 候选。
...
| `M7-002` | VoiceOver/TalkBack、动态字体、对比度和减少动态 | `NFR-018`–`NFR-020`、`AC-014` |
...
| `M7-006` | iOS/Android 支持矩阵和 `AC-001`–`AC-015` 全回归 | 发布门槛 |

### 退出门槛

- 全部 AC 与 NFR 验证通过，无未处理 critical/high 安全风险；
- 完整负载 30 分钟达标且无秘密扫描命中；
- 支持系统版本和无障碍矩阵通过；
- Preview 可安装、回滚、监控，隐私/资产/依赖清单已审核；
