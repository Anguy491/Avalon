...
| 微信小程序 E2E | miniprogram-automator + 开发者工具 | 构建渲染、路由、扫码入口、沙箱会话、前后台与音频入口 | PR 首页冒烟；真实 AppID 后核心集 |
...
`pnpm test:e2e:weapp` 使用微信开发者工具 CLI 做首页构建/渲染冒烟。真实 AppID 与合法 request/socket 域名就绪后，补充创建/加入、扫码拒绝与允许、沙箱会话恢复、后台中性遮罩、音频中断不重播和终局清理；规则组合仍由同一头部多客户端场景覆盖。小程序存储检查必须断言不存在 `RoomView`、角色、知识、票和任务行动。
