...
| 微信小程序 E2E | miniprogram-automator + 开发者工具 | 构建渲染、路由、扫码入口、双因子会话、私密层、前后台与音频入口 | PR 首页冒烟；合法域名/隔离后端就绪后核心集 |
...
- 所有 ErrorCode 到中英文可行动状态的映射；
- 系统语言偏好顺序、中文/英文变体、无匹配时英文回退，以及 catalog、角色、知识标签、阶段、胜因、配置错误和 `VoicePackKey` 的完整映射；
- 角色揭示隐私门、后台遮罩和提交后中性页；
...
Maestro 分别以 `zh_CN` 和 `en_US` 启动 iOS/Android 场景。英文流程至少覆盖首页、建房/加入、字段错误、相机拒绝、大厅、身份与攻略、对局、暂停和结果；中文运行现有核心回归。人工补充系统/单 App 语言切换后回前台的即时更新与会话保持，并保存英文正常状态、错误状态和最大字体截图。Expo Web 只要求同一语言解析保持可编译，不纳入本次原生验收。
...
`pnpm test:e2e:weapp` 使用微信开发者工具 CLI 做首页构建/渲染冒烟。合法 request/socket 域名和隔离后端就绪后，核心集覆盖创建/加入、全屏二维码、开始禁用原因、身份遮挡、私密投票/任务、暂停层、刺杀攻略、完整历史、扫码拒绝与允许、错微信账号、身份过期刷新、音频字幕降级/中断不重播和终局清理；规则组合仍由同一头部多客户端场景覆盖。小程序存储检查必须断言不存在微信身份令牌、`RoomView`、角色、知识、票和任务行动。
...
| `AC-017` | 引擎 + API | 5–10 人推荐牌组精确匹配且任务规则不变 |
| `AC-018` | 移动单元 + 原生 E2E | 八角色攻略只按本人角色显示并遵守私密生命周期 |
| `AC-019` | 微信身份单元 + API/DB/实时集成 + 双账号真机 | SessionToken 与微信身份双因子、一账号同房单座位、过期刷新和原生兼容 |
| `AC-020` | 本地化单元 + iOS/Android Maestro + 人工原生 | 系统偏好顺序、前台即时切换、会话不变、全量语义映射、中文音频配英文字幕、微信仍仅中文 |
...
| M7 Preview 就绪 | `AC-001`–`AC-013`、`AC-015`、`AC-019` 自动部分、故障注入、安全检查、30 分钟容量门槛；不得记为已部署 |
